 ...
 *  @return     0 if successful.
 */
int mpu_get_temperature(int16_t *data, unsigned long *timestamp)
{
    unsigned char tmp[2];
    int16_t raw;
    ...
    // data[0] = (int16_t)((35 + ((raw - (float)st.hw->temp_offset) / st.hw->temp_sens)) * 65536L);
    *data = (((float)raw) - (float)st.hw->temp_offset)/st.hw->temp_sens + 21.0;
    return 0;
}
