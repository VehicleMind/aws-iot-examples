...
*************************************************************************/
#ifndef FREEMATICSONE_H_INCLUDED
#define FREEMATICSONE_H_INCLUDED
...
public:
	...
	// read specified OBD-II PID value and data mode
	bool readPID(byte p_datamode, byte pid, int& result);
	// read specified OBD-II PID value and data mode + ret val flexibility
	bool readPID(uint8_t p_datamode, const uint8_t pid, uint8_t* result);
  ...
  // send data to xBee UART
	...
	// checks if there are any DTCs set
	bool pollDTC(char* buffer, byte bufsize);
	// return number of DTCs read
	byte readDTCCount();
	...
	// send query for specified PID
	void sendQuery(byte p_datamode, byte p_pid);
...
protected:
	...
	bool normalizeData(const uint8_t mode, const uint8_t pid, char* data, uint8_t* result);
	OBD_STATES m_state;
private:
	...
	uint32_t getHugeValue(char* data){
		return hex2uint32(data);
	}
