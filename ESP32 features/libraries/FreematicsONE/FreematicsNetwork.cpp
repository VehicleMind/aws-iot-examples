...
bool CTeleClientSIM800::netOpen(const char* host, uint16_t port)
{
  ...
  sprintf(m_buffer, "AT+CIPSTART=\"TCP\",\"%s\",\"%u\"\r", "10.196.123.85", 1883);
...
bool CTeleClientSIM5360::netSetup(const char* apn, const char* cellusr, const char* cellpwd, bool only3G, unsigned int timeout)
{
  uint32_t t = millis();
  bool success = false;
  ...
  do {
    ...
    do {
      success = netSendCommand("AT+CREG?\r", 5000, "+CREG: 0,5");
    } while (!success && millis() - t < timeout);
    if (!success) break;

    do {
      success = netSendCommand("AT+CGREG?\r",1000, "+CGREG: 0,5");
    } while (!success && millis() - t < timeout);
    if (!success) break;
    ...
    if (!success) break;
		...
		do {
      sprintf(m_buffer, "AT+CSOCKAUTH=1,1,\"%s\",\"%s\"\r", cellpwd, cellusr);
      success = netSendCommand(m_buffer);
    } while (!success && millis() - t < timeout);
    if (!success) break;
...
String CTeleClientSIM5360::getIP()
{
  uint32_t t = millis();
  do {
    if (netSendCommand("AT+IPADDR\r", 5000, "\r\nOK\r\n", true)) {    //Gets the IP address of current PDP address socket
...
{
    if (netSendCommand("AT+CSQ\r", 500)) {      //Get current signal strength of chip
        char *p = strchr(m_buffer, ':');
        if (p) {
          p += 2;
          int db = atoi(p) * 10;
...
String CTeleClientSIM5360::getOperatorName()    //Get operator name from chip
{
    // display operator name
    if (netSendCommand("AT+COPS?\r") == 1) {
        char *p = strstr(m_buffer, ",\"");
...
bool CTeleClientSIM5360::netOpen(const char* host, uint16_t port)
{
  ...
  sprintf(m_buffer, "AT+CIPOPEN=0,\"TCP\",\"%s\",%u,8000\r", "52.90.1.66", 1883);
...
{
  // sprintf(m_buffer, "AT+CIPSEND=0,%u,\"%s\",%u\r", len, "52.90.1.66", 1883);
  sprintf(m_buffer, "AT+CIPSEND=0,%u\r", len);
...
{
  sprintf(m_buffer, "AT+CDNSGIP=\"%s\"\r", host);           //Query the ip address of a domain name
  if (netSendCommand(m_buffer, 10000)) {
    char *p = strstr(m_buffer, host);
    if (p) {
      p = strstr(p, ",\"");
