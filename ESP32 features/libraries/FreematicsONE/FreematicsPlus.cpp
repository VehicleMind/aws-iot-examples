...
void gps_decode_task(int timeout)
{
    ...
    uint8_t c;
    // Serial.println("attempting to read gps");
    int len = uart_read_bytes(GPS_UART_NUM, &c, 1, timeout / portTICK_RATE_MS);
    // Serial.println("Read bytes from gps uart");
    if (len == 1)
    {
        // Serial.printf("encode gps with char %c\n",c);
        if (gps->encode(c))
        {
            // Serial.println("gps encode success.");
            newGPSData = true;
        }
    }
}
...
int bee_read(uint8_t* buffer, size_t bufsize, unsigned int timeout)
{
    ...
    do {
        ...
        gps_decode_task(0);
        if (len == 1) {
            //JAMES GOT RID OF THIS SO MQTT WOULD WORK
            // if (c >= 0x0A && c <= 0x7E) {
                buffer[recv++] = c;
            // }
        } else if (recv > 0) {
