...
{
     ori->yaw  = atan2f(2.0f * (q[1] * q[2] + q[0] * q[3]), q[0] * q[0] + q[1] * q[1] - q[2] * q[2] - q[3] * q[3]) * 180.0f / PI;
     if(ori->yaw < 0) ori->yaw   += 360.0f; // Ensure yaw stays between 0 and 360
     ori->pitch = -asinf(2.0f * (q[1] * q[3] - q[0] * q[2])) * 180.0f / PI;
     ori->roll  = atan2f(2.0f * (q[0] * q[1] + q[2] * q[3]), q[0] * q[0] - q[1] * q[1] - q[2] * q[2] + q[3] * q[3]) * 180.0f / PI;
}
...
bool MPU9250_9DOF::initAK8963(float * destination)
{
  if (readByteAK(WHO_AM_I_AK8963) != 0x48) {
      Serial.println("MEMS CAL FAILS AT 'WHO AM I'"); 
      return false;
  }
...
void MPU9250_9DOF::magcalMPU9250(float * magBias, float * magScale){
...
bool MPU9250_9DOF::read(float* acc, float* gyr, float* mag, int16_t* temp, ORIENTATION* ori)
{
  ...
  if (quaterion && acc && gyr && mag) {
    quaterion->MadgwickQuaternionUpdate(-acc[0], acc[1], acc[2], gyr[0]*PI/180.0f, -gyr[1]*PI/180.0f, -gyr[2]*PI/180.0f,  mag[1],  -mag[0], mag[2]);
