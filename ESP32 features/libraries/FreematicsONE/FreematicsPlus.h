...
*************************************************************************/
#ifndef FREEMATICSPLUS_H_INCLUDED
#define FREEMATICSPLUS_H_INCLUDED
...
public:
    ...
    virtual void jLog(int32_t value, bool is_PID){
      char buf[12];

      byte len;

      if(is_PID){
        len = sprintf_P(buf, PSTR("%X"), value);
      } else {
        len = sprintf_P(buf, PSTR("%ld"), value);
      }
