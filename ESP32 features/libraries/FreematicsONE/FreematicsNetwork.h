...
#ifndef FREMATICSNETWORK_H_INCLUDED
#define FREMATICSNETWORK_H_INCLUDED
...
public:
    bool netBegin();
    void netEnd();
    bool netSetup(const char* apn, const char* cellusr, const char* cellpwd,  bool only3G = false, unsigned int timeout = 60000);
    String getIP();
    int getSignal();
    String getOperatorName();
    bool netOpen(const char* host, uint16_t port);
